# model.py
# Orquestador V15.6 (Platoon Splits, Momentum, Pitagóricos y VMR Dinámico)
...
class MLBPredictor:
    ...
    def predict_game(self, game):
        ...
        # 2. Pitcheo, Bullpen y Mano del Lanzador (Para Platoon Splits)
        ...
        # 3. Lineups (Sin inyecciones falsas para el pasado)
        ...
        h_rs, h_ra = self.loader.get_team_pythagorean_data(game['home_id'])
        a_rs, a_ra = self.loader.get_team_pythagorean_data(game['away_id'])
        ...
        h_power = self.engine.calculate_power_score(h_ops_adj, pf, league_avg_runs, game['home_id'], game['date'], schedule_df) * h_streak_mult
        a_power = self.engine.calculate_power_score(a_ops_adj, pf, league_avg_runs, game['away_id'], game['date'], schedule_df) * a_streak_mult
        ...
        # --- NUEVO: VMR Dinámico basado en K/9 ---
        avg_k9 = (h_pstats['k9'] + a_pstats['k9']) / 2.0
        k9_vmr_adj = 1.0 + ((7.5 - avg_k9) * 0.02)

        # 8. Simulación Estocástica Principal (Pasando el k9_vmr_adj)
        win_prob, h_runs, a_runs, _ = self.engine.run_monte_carlo_simulation(
            ...
            k9_adj=k9_vmr_adj # <--- Parámetro inyectado
        )

        # 9. Análisis de Sensibilidad (Stress Testing)
        delta_h = h_ops_adj * 0.05
        delta_a = a_ops_adj * 0.05
        
        h_pow_high = self.engine.calculate_power_score(h_ops_adj + delta_h, pf, league_avg_runs, game['home_id'], game['date'], schedule_df) * h_streak_mult
        a_pow_low = self.engine.calculate_power_score(a_ops_adj - delta_a, pf, league_avg_runs, game['away_id'], game['date'], schedule_df) * a_streak_mult
        ...
        h_pow_low = self.engine.calculate_power_score(h_ops_adj - delta_h, pf, league_avg_runs, game['home_id'], game['date'], schedule_df) * h_streak_mult
        a_pow_high = self.engine.calculate_power_score(a_ops_adj + delta_a, pf, league_avg_runs, game['away_id'], game['date'], schedule_df) * a_streak_mult
